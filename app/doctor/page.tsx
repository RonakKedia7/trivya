  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Welcome, {currentDoctor.name}</h1>
          <p className="text-muted-foreground">{currentDoctor.specialization} - {currentDoctor.department}</p>
        </div>
      ...
      <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {[
          { label: "Today's Appointments", value: stats?.todayAppointments ?? 0, color: "chart-1" },
          { label: "Upcoming", value: stats?.upcomingAppointments ?? 0, color: "warning" },
          { label: "Completed", value: stats?.completedAppointments ?? 0, color: "success" },
          { label: "Total Patients", value: stats?.totalPatients ?? 0, color: "chart-2" },
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-border bg-card">
          <div className="flex items-center justify-between border-b border-border px-6 py-4">
            ...
            <Link href="/doctor/appointments" className="cursor-pointer text-sm text-primary hover:underline">View all</Link>
          </div>
          {todayApts.length > 0 ? (
            ...
            <div className="p-6 text-center text-muted-foreground">No appointments scheduled for today.</div>
          )}
        </div>
        ...
        <div className="rounded-xl border border-border bg-card">
          ...
          {upcomingApts.length > 0 ? (
            ...
            <div className="p-6 text-center text-muted-foreground">No upcoming appointments.</div>
          )}
        </div>
      </div>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        {[
          { href: "/doctor/availability", label: "Manage Availability", sublabel: "Update your schedule", color: "chart-1" },
          { href: "/doctor/medical-records", label: "Medical Records", sublabel: "Create new records", color: "chart-2" },
          { href: "/doctor/history", label: "Patient History", sublabel: "View all patients", color: "success" },
        ].map(({ href, label, sublabel, color }) => (
          <Link key={href} href={href} className="flex cursor-pointer items-center gap-4 rounded-xl border border-border bg-card p-6 transition-colors hover:bg-secondary/50">
