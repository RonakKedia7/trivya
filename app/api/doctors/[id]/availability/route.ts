...
function normalizeSchedule(raw: any) {
  if (typeof raw === 'boolean') {
    if (raw) return WEEK_DAYS.map((day) => ({ day, isWorking: ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'].includes(day), slots: [] }));
  ...
  for (const item of incoming) {
    ...
    const slots = Array.isArray(item?.slots)
      ? item.slots
          ...
          .map((s: any) => ({ startTime: s.startTime.trim(), endTime: s.endTime.trim(), isAvailable: Boolean(s.isAvailable) }))
      : [];
  ...
  try {
    ...
    for (const day of normalizedSchedule) {
      const ranges = day.slots
        .map((slot: any) => ({ start: toMinutes(slot.startTime), end: toMinutes(slot.endTime) }))
        .filter((r: any) => r.start !== null && r.end !== null);
      for (let i = 0; i < ranges.length; i += 1) {
        if (ranges[i].start >= ranges[i].end) return badRequest('Invalid availability payload or overlapping time slots');
        for (let j = i + 1; j < ranges.length; j += 1) {
          const overlap = ranges[i].start < ranges[j].end && ranges[j].start < ranges[i].end;
          if (overlap) return badRequest('Invalid availability payload or overlapping time slots');
        }
      }
    }
    const updatedDoc = await DoctorModel.findByIdAndUpdate(id, { availability: normalizedSchedule }, { new: true }).select('availability');
    const updated = updatedDoc ? normalizeSchedule(updatedDoc.availability) : null;
